...
<main class="max-w-7xl mx-auto px-4 py-8">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div>
      <div class="bg-white rounded-lg shadow-md p-4">
        <img src="<?= htmlspecialchars($product['image']) ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="w-full main-image">
    ...
    <div>
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="flex items-center mb-4">
          ...
          <span class="text-sm text-gray-500">(<?= htmlspecialchars($product['rating']) ?>)</span>
        ...
        <div class="mb-6">
          ...
          <?php if (isset($product['original_price'])): ?>
            <p class="text-sm text-gray-500">
              ...
              <span class="text-green-600 ml-2"><?= round(100 - ($product['price'] / $product['original_price'] * 100)) ?>% off</span>
            </p>
          <?php endif; ?>
        </div>
        ...
        <div class="mb-6">
          ...
          <ul class="list-disc pl-5 text-gray-700 space-y-1">
            <?php if (isset($product['highlights']) && is_array($product['highlights'])): ?>
        ...
        <div class="mb-6">
          <div class="flex items-center space-x-4">
            <form method="post" action="add_to_cart.php" class="flex-1">
              <input type="hidden" name="product_id" value="<?= htmlspecialchars($product['name']) ?>">
  ...
  <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
    ...
    <div class="md:col-span-2 bg-white rounded-lg shadow-md p-6">
      ...
      <div class="prose max-w-none text-gray-700">
        ...
        <?php else: ?>
          <p>This premium auto part is designed to meet or exceed OEM specifications, ensuring optimal performance and longevity. Manufactured with high-quality materials and precision engineering, it provides a perfect fit and reliable operation for your vehicle.</p>
          <p class="mt-4">The product comes with a comprehensive warranty and is backed by our customer satisfaction guarantee. Easy to install and maintain, this part will help keep your vehicle running smoothly for years to come.</p>
        <?php endif; ?>
      </div>
    </div>
      ...
      <table class="w-full specs-table">
        <tbody>
          <tr>
            ...
            <td class="text-gray-900"><?= htmlspecialchars($product['brand']) ?></td>
          </tr>
          <tr>
            <td class="font-medium text-gray-500">Category</td>
            <td class="text-gray-900"><?= htmlspecialchars($product['category']) ?></td>
          </tr>
          <tr>
            <td class="font-medium text-gray-500">Part Number</td>
            <td class="text-gray-900"><?= htmlspecialchars($product['part_number'] ?? 'N/A') ?></td>
          </tr>
          <tr>
            <td class="font-medium text-gray-500">Compatibility</td>
            <td class="text-gray-900">
              <?php if (isset($product['compatibility']) && is_array($product['compatibility'])): ?>
                <?= implode(', ', array_map('htmlspecialchars', $product['compatibility'])) ?>
              <?php else: ?>
          ...
          <tr>
            <td class="font-medium text-gray-500">Weight</td>
            <td class="text-gray-900"><?= htmlspecialchars($product['weight'] ?? 'N/A') ?></td>
          </tr>
          <tr>
            <td class="font-medium text-gray-500">Dimensions</td>
            <td class="text-gray-900"><?= htmlspecialchars($product['dimensions'] ?? 'N/A') ?></td>
          </tr>
          <tr>
            <td class="font-medium text-gray-500">Material</td>
            <td class="text-gray-900"><?= htmlspecialchars($product['material'] ?? 'N/A') ?></td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <div class="mt-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      $relatedProducts = array_filter($partsData, function($item) use ($product) {
      ...
      if (count($relatedProducts) > 0): 
        foreach ($relatedProducts as $related): ?>
          <a href="product_details.php?id=<?= urlencode($related['name']) ?>" class="bg-white rounded-xl shadow-md p-4 flex flex-col items-start hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            <div class="relative w-full">
              <img src="<?= htmlspecialchars($related['image']) ?>" alt="<?= htmlspecialchars($related['name']) ?>" class="w-full h-48 object-contain rounded-md mb-3">
            ...
            <p class="text-sm text-gray-600 mb-1">Brand: <span class="font-medium"><?= htmlspecialchars($related['brand']) ?></span></p>
            <div class="flex items-center mb-2">
              <div class="flex text-yellow-400">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                  <?php if ($i <= floor($related['rating'])): ?>
                    <i class="fas fa-star"></i>
                  <?php elseif ($i - 0.5 <= $related['rating']): ?>
                    <i class="fas fa-star-half-alt"></i>
                  <?php else: ?>
                    <i class="far fa-star"></i>
                  <?php endif; ?>
              ...
              <span class="text-sm text-gray-500 ml-1">(<?= htmlspecialchars($related['rating']) ?>)</span>
            </div>
