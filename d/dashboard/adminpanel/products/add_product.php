...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($_FILES['image']['name'])) {
            $uploadDir = '../../uploads/products/'; // Adjusted path based on your structure
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadFile)) {
                $imagePath = 'uploads/products/' . $fileName; // Path relative to root
            } else {
                throw new Exception("Error uploading file. Please check directory permissions.");
            }
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php if (!empty($formData['image_path'])): ?>
                <div>
                    ...
                    <?php if (strpos($formData['image_path'], 'http') === 0): ?>
                        <!-- External URL -->
