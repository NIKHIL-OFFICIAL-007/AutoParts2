...
<nav class="bg-primary text-white px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-lg">
  ...
  <div class="flex flex-1">
    ...
    <div id="sidebar" class="w-64 bg-white shadow-lg overflow-y-auto z-40 scrollbar-hide">
      ...
      <div class="sidebar-section">
        ...
        <div class="price-display flex justify-between mb-2">
          <div class="text-sm">$<span id="minPriceDisplay"><?= $minPrice ?></span></div>
          <div class="text-sm">$<span id="maxPriceDisplay"><?= $maxPrice ?></span></div>
        </div>
        
        <div class="slider-container">
          <div class="slider-track" id="sliderTrack"></div>
          <input type="range" min="<?= $minPrice ?>" max="<?= $maxPrice ?>" value="<?= $minPrice ?>" 
                 class="slider-range-input" id="minSlider">
          <input type="range" min="<?= $minPrice ?>" max="<?= $maxPrice ?>" value="<?= $maxPrice ?>" 
                 class="slider-range-input" id="maxSlider">
        </div>
        
        <div class="price-inputs flex justify-between mt-4">
          <div class="w-45percent">
            <label for="minInput" class="block text-xs text-gray-600 mb-1">Min</label>
            <input type="number" id="minInput" min="<?= $minPrice ?>" max="<?= $maxPrice ?>" value="<?= $minPrice ?>" 
                   class="price-input w-full text-sm" onchange="validateMinPrice()">
          </div>
          <div class="w-45percent">
            <label for="maxInput" class="block text-xs text-gray-600 mb-1">Max</label>
            <input type="number" id="maxInput" min="<?= $minPrice ?>" max="<?= $maxPrice ?>" value="<?= $maxPrice ?>" 
                   class="price-input w-full text-sm" onchange="validateMaxPrice()">
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    let minPriceDisplay = document.getElementById('minPriceDisplay');
    let maxPriceDisplay = document.getElementById('maxPriceDisplay');
    ...
    function initPriceRange() {
      ...
      minInput.addEventListener('change', validateMinPrice);
      maxInput.addEventListener('change', validateMaxPrice);
